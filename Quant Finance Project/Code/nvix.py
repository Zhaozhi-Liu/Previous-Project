...
class NVIX:
    ...
    def __init__(self,maturity,heston = None):
        if heston:
            self.r = 0
            self.T = 30/365
            self.calls = heston["call"]
            self.puts = heston["put"]
        else:
            self.today = datetime.strptime(datetime.now().strftime("%Y-%m-%d"), "%Y-%m-%d")
            self.spx = yf.Ticker("^SPX") 
            self.r = yf.Ticker("^IRX").history(period="5d")["Close"].iloc[-1]*0.01 # risk-free rate
            self.days = abs((maturity - self.today).days) # days to expire
        ...
        self.k0,self.call_price,self.put_price = self._k0() # ATM strike; ATM call price; ATM put price
        self.table,self.points = self.construct_dataframe() # used to calculate vol
        self.variance = self.vol() # variance of log quadratic variation
        
    def _k0(self):
        call_df = self.calls.loc[:,["strike","lastPrice"]]
        put_df = self.puts.loc[:,["strike","lastPrice"]]
    ...
    # formula defined as similar in the paper but in terms of call price
    ...
    # construct the coefficients data as presented in the paper
    ...
    # the interpolated function of (d2,sigma^2)
    def _y(self,x):
        df_combine = self.table.copy()
    ...
    # plot the interpolated graph as in the paper
    def graph(self):
    ...
    # calculate the variance using the new method
    def vol(self):
        df_combine = self.table.copy()
